...
exports.protect = catchAsync( async (req, res, next) => {
  ...
  if(!token || !token.startsWith('Bearer')) return next( new appError("To access this route, please login first", 401));
  ...
  if(!VerifiedUser) return next( new appError('User does not exist, login again!', 401))
  ...
  if(VerifiedUser.changedPasswordAt){
    ...
    if(changedTime>issuedTime) return next( new appError('User has changed the password, please login again!'));
...
exports.forgotPassword = catchAsync( async (req, res, next) =>{
 ...
 await user.save({validateBeforeSave: false});
  ...
  await sendEmail({
    email: user.email,
    subject: "Reset Your IBDB Password (valid for 10 mins)",
    text:resetUrl
  })
...
exports.resetPassword = catchAsync (async (req, res, next) =>{
  ...
  const user = await User.findOne({passwordResetToken:encToken, resetTokenExpiresIn:{$gt:Date.now()}});
