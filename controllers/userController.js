...
exports.updateMe = catchAsync (async (req, res, next) =>{
    ...
    if(Object.keys(req.body).length == 0) return next(new appError('There is nothing to update', 401));
...
exports.deleteMe = catchAsync (async (req, res, next) =>{
    ...
    const user = await User.findByIdAndUpdate({_id:req.user.id},{active:false});
    res.status(200).send();
})